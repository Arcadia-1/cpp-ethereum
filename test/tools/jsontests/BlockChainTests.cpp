    ...
    for (auto const& i : _input.get_obj())
    {
        ...
        else
        {
            BOOST_REQUIRE_MESSAGE(
                inputTest.count("network"), "\"network\" field is not found. filename: " +
                                                TestOutputHelper::get().testFile().string() +
                                                " testname: " + TestOutputHelper::get().testName());
...
json_spirit::mObject fillBCTest(json_spirit::mObject const& _input)
{
    ...
    for (auto const& bl : _input.at("blocks").get_array())
    {
        ...
        try
        {
            ...
            bool isException =
                (blObjInput.count("expectException" +
...
void overwriteBlockHeaderForTest(mObject const& _blObj, TestBlock& _block, ChainBranch const& _chainBranch)
{
    ...
    if (ho.size() != 14)
    {
        ...
        Ethash::setMixHash(
            tmp, ho.count("mixHash") ? h256(ho["mixHash"].get_str()) : Ethash::mixHash(header));
        Ethash::setNonce(
            tmp, ho.count("nonce") ? eth::Nonce(ho["nonce"].get_str()) : Ethash::nonce(header));
        tmp.noteDirty();
    }
    else
...
void checkBlocks(TestBlock const& _blockFromFields, TestBlock const& _blockFromRlp, string const& _testname)
{
    ...
    BOOST_CHECK_MESSAGE(blockHeaderFromFields.hash(WithoutSeal) == blockFromRlp.hash(WithoutSeal),
    ...
    BOOST_CHECK_MESSAGE(blockHeaderFromFields.transactionsRoot() == blockFromRlp.transactionsRoot(),
